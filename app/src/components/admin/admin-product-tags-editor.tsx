...
interface AdminProductTagsEditorProps {
  productId: string;
}

export function AdminProductTagsEditor({
  productId,
}: AdminProductTagsEditorProps) {
  ...
  const fetchTags = async () => {
    ...
    try {
      ...
      const currentTagIds = productTagsData?.map(pt => pt.tag_id) || [];
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Delete all existing product tag associations
    ...
    return (
      <div className='p-8 bg-gray-50 rounded-lg border border-gray-200 text-center'>
        ...
        <p className='text-gray-600 mb-4'>
          Create tags in Settings → Products → Tags before assigning them to
          products.
        </p>
      </div>
    );
