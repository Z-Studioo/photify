...
export function ImportConfirmationModal({
  ...
  return (
    <ConfirmationModal
      ...
      title={isInvalid ? "Invalid Configuration File" : "Import Preset Configuration"}
      confirmText={isImporting ? "Importing..." : (isInvalid ? "Re-upload File" : "Confirm Upload")}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-md p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-gray-500">{importSummary.fileSize}</span>
            </div>
            <p className="text-sm text-gray-600 break-all">{importSummary.fileName}</p>
          </div>
          ...
          {isInvalid ? (
            <div className="bg-red-50 border border-red-200 rounded-md p-3">
              ...
              <p className="text-xs text-red-600 mt-1">
                {importSummary.error || "This is not a valid Photify preset file."}
              </p>
            ...
            <>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-600">{importSummary.frameSize}</span>
                  </div>

                  {/* Ratio */}
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">Aspect Ratio</span>
                    <span className="text-xs text-gray-600">{importSummary.frameRatio}</span>
                  </div>

                  {/* Edge Type */}
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">Edge Type</span>
                    <span className="text-xs text-gray-600 capitalize">{importSummary.edgeType}</span>
                  </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
                <p className="text-xs text-yellow-800">
                  ⚠️ This will overwrite your current frame configuration{importSummary.hasImage ? ' and image' : ''}.
                </p>
              </div>
            </>
          )}
