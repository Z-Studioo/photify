...
const shapes: { id: CanvasShape; name: string; icon: React.ComponentType<React.SVGProps<SVGSVGElement>> }[] = [
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {shapes.map((shapeOption) => (
          ...
          >
            ...
            <span className="text-xs font-medium text-center">{shapeOption.name}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 p-3 bg-gray-50 rounded-lg">
        <p className="text-xs text-gray-600">
          <strong>Selected:</strong> {shapes.find(s => s.id === shape)?.name || 'Rectangle'}
        </p>
